defmodule TullnData.Pegelonline do
  @moduledoc """
  ...
  def station_keys, do: Map.keys(@stations)

  def current_water_level(station_key) when is_map_key(@stations, station_key) do
  ...
  def measurements(station_key, duration \\ "P7D")
      when is_map_key(@stations, station_key) do
    station = @stations[station_key]
  ...
  def characteristic_values(station_key) when is_map_key(@stations, station_key) do
    station = @stations[station_key]
